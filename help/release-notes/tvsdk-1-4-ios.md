...
PTSDKConfigクラスは、Adobe PrimetimeAd Decisioning、DRMおよびVideo Analyticsサーバーに対する要求でSSLを強制するメソッドを提供します。 詳しくは、このクラスの`forceHTTPS`メソッドと`isForcingHTTPS`メソッドを参照してください。 マニフェストがHTTPS経由で読み込まれる場合、TVSDKはHTTPSのコンテンツ使用を保持し、そのマニフェストから相対URLを読み込む際にこの使用状況を順守します。
...
VPAID 2.0について詳しくは、[VPAID広告のサポート](../programming/tvsdk-1.4-for-ios/ad-insertion/c-psdk-ios-1.4-vpaid-2.0-ads.md)を参照してください。
...
詳しくは、[VASTおよびVMAP広告に対する広告のフォールバック](../programming/tvsdk-1.4-for-ios/ad-insertion/c-psdk-ios-1.4-ad-fallback.md)を参照してください。
...
* iOS 12でのチケット#36493 - `libstdc++`のサポート
...
* (ZD#33341) - DRMInterface.hトリガーのビルド警告はXcode 9で行います。
...
* (ZD #32465) — プレイヤーは、結合された再生リストを処理できません。
   ...
   finishLoadingWithError(with:エラー) AVファンデーションが代替ストリーム/トリガーのフェイルオーバーを試行するためのエラー。
...
* (ZD #33336) - [iOS] TVSDK — 広告ポッドが満たされません。Freewheelから返される広告が十分にあります。
...
詳しくは、[請求指標](../programming/tvsdk-1.4-for-ios/c-psdk-ios-1.4-billing/c-psdk-ios-1.4-billing.md)を参照してください。
...
この問題は、assetDuration APIを`PTVideoAnalyticsTrackingMetadata`に追加して、ライブ/リニアストリームのアセット期間を更新し、ライブストリームを確認するロジックを提供することで解決されました。
...
* Zendesk #18518 — ビデオ名が指定されていない場合、TVSDKの名前はデフォルトで&#x200B;*PSDKベースのプレイヤーに設定されます。*
...
* 最小ターゲットがiOS7（またはそれ以降）に設定されている場合、drmNativeInterface.frameworkは正しくリンクされません。\
   回避策：`libstdc++6`を明示的に指定します。  dylibライブラリを次のように指定します。「ターゲット」>「Build Phases」>「Link Binary with Libraries」に移動し、`libstdc++.6.dylib`を追加します。
