...
1. Metadata インスタンスを渡します。このインスタンスは、 `TimeRangeCollection` インスタンス、 `replaceCurrentItem` メソッド（MediaPlayer インターフェイスの一部）を使用します。
1. TVSDK が `PREPARED` ～を待つことによって国家を作る `PlaybackEventListener#onPrepared` コールバックをトリガーします。
...
次のコードスニペットは、3 つの TimeRange 指定のセットをカスタム広告マーカーとしてタイムラインに配置する簡単な例です。
