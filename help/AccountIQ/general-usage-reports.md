...
* **ユニーク購読者**
   ...
   >[!NOTE]
   >
   >プログラマーがAdobeTempPass （つまり、空きプレビュー）を使用しているデバイスがセグメントに含まれる場合、ユニーク購読者の合計数には、ユニークデバイスの数も含まれます。
...
この **棒グラフ** は、MVPD の購読者がデバイスを使用してチャネルからのコンテンツにアクセスする方法に関する、使用状況に関するインサイトを提供します。
...
（MVPD とチャネルを選択して）セグメントを定義し、場所の数のしきい値を設定したら、グラフから次の項目を識別できます。
...
>[!MORELIKETHIS]
>
>* 一般使用状況レポートで、フィルターを使用して、選択した MVPD の上位 1000 人の購読者に対してレポートを書き出す方法を説明します。 [上位 1000 件のアカウントのエクスポート](/help/AccountIQ/export-acc-information.md) オプション。
