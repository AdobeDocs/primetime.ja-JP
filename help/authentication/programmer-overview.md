...
Adobe Primetime認証エンタイトルメントソリューションでは、認証および承認ワークフローが正常に完了すると作成される特定のデータの生成が中心となります。 これらのデータはトークンと呼ばれます。 トークンの有効期限は限られています。トークンが期限切れになったら、認証および承認ワークフローの再開を通じてトークンを再発行する必要があります。
...
* **AuthN トークン** （「長期間有効」）：認証が成功すると、Adobe Primetime認証によって、リクエストデバイスとグローバル一意識別子 (GUID) の両方に関連付けられた AuthN トークンが作成されます。
...
* **AuthZ トークン** （「長期間有効」）：認証が成功すると、Adobe Primetime認証によって、リクエストデバイスと特定の保護されたリソースに関連付けられた AuthZ トークンが作成されます。  保護されたリソースは、一意のリソース ID で識別されます。
   * Adobe Primetime認証は、AuthZ トークンを Access Enabler に送信し、AuthZ トークンをローカルシステム上に安全にキャッシュします。 次に、Access Enabler は AuthZ トークンを使用して、実際の表示アクセスに使用される短時間のみ有効なメディアトークンを作成します。
