...
1. 時間が実行さ `TimedMetadata` れたときにオブジ `MediaPlayer` ェクトを処理します。
...
1. ブラックアウト範囲が再生ストリームのDVRにある場合は、シーク不可能範囲を更新します。
   ...
   >[!NOTE]
   >
   >現在、複数のビットレートライブストリームの場合、可変ビットレート(ABR)プロファイルが同期しない場合があります。 これにより、同じサブスクラ `timedMetadata` イブ済みタグに対してオブジェクトが重複します。 シーク不可能な計算が正しく行われないように、次の例のように、計算の後に、シーク不可能な範囲が重なっていないかどうかを確認することを強くお勧めします。
