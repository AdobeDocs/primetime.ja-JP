...
* (ZD#32990) - iOS：一部のキューポイントで、広告の代わりにコンテンツを再生しています。 `selectedMediaOptionInMediaSelectionGroup` AVPlayerItem インターフェイスの一部であった API が、以下に移動しました。 `AVMediaSelection` iOS 11. この新しい API を使用して問題を解決しました。
