...
このバージョンでは、クリエイティブタグが見つからない場合や、 [!UICONTROL url CDATA] 次の項目が空です： [!UICONTROL VAST] 応答。
...
* **HTTPS 経由でのセキュア配信** - Android TVSDK 3.9 は、HTTPS を介して安全な配信機能を導入し、卓越したスケールとパフォーマンスでコンテンツを安全に配信します。
  ...
  HTTPS 経由でのセキュアな配信を有効にするために、新しい API が `NetworkConfiguration` クラス。
...
* **API の変更点**
   ...
   * 新しい API が `NetworkConfiguration::setNumOfTimesManifestRetryBeforeError(n)*` ネットワークのエラーとタイムアウトを処理する。
      * (n) は再試行の回数です。
...
遅延広告解決は、各広告ブレークを個別に解決するようになりました。 以前は、広告の解決は 2 段階のアプローチでした。再生開始前にプリロールを解決し、再生開始後に組み合わされたすべてのミッド/ポストロールスロットを解決しました。 この強化された機能を使用すると、各広告ブレークが、広告キューポイントの前の特定の時間に解決されるようになりました。
...
詳しくは、 [Android 向け TVSDK 3.0 プログラマーガイド](../programming/tvsdk-3x-android-prog/android-3x-advertising/ad-insertion/c-lazy-ad-resolving/c-lazy-ad-resolving.md) API とイベントの変更時に有効です。
...
* **Placement.Type getPlacementType() は、インターフェイス TimelineMarker のメソッドになりました。**
  ...
  このメソッドは、Placement.Type.PRE_ROLL、Placement.Type.MID_ROLL または Placement.Type.Placement_ROLL の配置タイプを返します。POSTタイプは以下の通りです。 広告ブレークが未解決の場合、 TimelineMarker インターフェイスの getDuration() メソッドは 0 を返します。
...
* CRS を使用する TVSDK のすべてのお客様は、Android で TVSDK 2.5.3.85（最新）を使用してアプリをアップグレードすることをお勧めします。 これは、既存のアプリケーションの実装に対するドロップインの代わりとなります。 TVSDK のアップグレード後、プロキシツールで CRS クリエイティブ URL リクエストを確認し（例：Charles）、パス内のホスト名とバージョンが、以下のサンプル URL 構造と同じように反映されていることを確認します。
...
* Android アプリケーションと TVSDK 間での Cookie の共有：Android TVSDK で、JAVA レイヤー（Android アプリケーションの CookieStore に格納）と C++ TVSDK レイヤー間での Cookie のアクセスがサポートされるようになりました。 これで、Java Cookie Store に公開される C++のネイティブレイヤーで cookie を設定または変更できるようになりました。
...
* 変更点： `SizeAvailableEvent`. 以前は、 `getHeight()` および `getWidth()` ～の方法 `SizeAvailableEvent` 2.5.2 では、フレームの高さとフレーム幅を返します。これはメディア形式で返されます。 現在は、デコーダーから返される出力の高さと出力の幅を返します。
...
* **SizeAvaliableEventListener**
   ...
   * `getHeight()` および `getWidth()` ～の方法 `SizeAvailableEvent` 出力をそれぞれ高さと幅で返すようになりました。 表示縦横比は、次のように計算できます。
...
* ZD#46903 — クリエイティブタグが見つからない場合、またはクリエイティブタグが見つからない場合に、アプリケーションが複数回クラッシュする [!UICONTROL url CDATA] 次の項目が空です： [!UICONTROL VAST] 応答。
...
* Zendesk#31806 - PREPARING でのAuditudeスティック
