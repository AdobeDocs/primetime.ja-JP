...
| 暗黙的で均一なデバイス検出 | プログラマー | SDK は、デバイスタイプを自動的に検出し、その情報を標準的な方法で送信します。 REST API 実装では、実装者に応じて異なる情報が送信されがちなので、サイトをまたいでビジネスルールや統計を偏らせる傾向があります。 **プログラマーは、送信された内容が正しいことを確認する必要があります [デバイス情報](http://tve.helpdocsonline.com/passing-device-information) それぞれの** アプリ サーバー間実装の場合、追加の手順がおこなわれない限り、REST API はエンドユーザーの IP アドレスを正しく識別しません。 IP アドレスは、不正防止や HBA などの特定の使用事例で重要です。 詳しくは、 [サーバー側実装でのクライアント IP アドレスの受け渡し方法](http://tve.helpdocsonline.com/clientless-integration-cookbook-v2$overall_clientIP). |
