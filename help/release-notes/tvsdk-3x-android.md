...
seo-title: TVSDK 3.10 for Androidリリースノート
description: TVSDK 3.10 for Androidリリースノートでは、TVSDK Android 3.10の新機能や変更点、解決済みで既知の問題、デバイスの問題について説明します。
seo-description: TVSDK 3.10 for Androidリリースノートでは、TVSDK Android 3.10の新機能や変更点、解決済みで既知の問題、デバイスの問題について説明します。
...
現在のリリースでは、「解決された問題」の節で説明した、お客様の主な問題の修正に [重点を置いて](#resolved-issues) います。
...
* **HTTPS経由のセキュア配信** - Android TVSDK 3.9では、HTTPS経由のセキュア配信機能が導入され、比類のない規模とパフォーマンスでコンテンツを安全に配信できます。
   ...
   HTTPS経由の安全な配信を有効にするため、新しいAPIがクラスに導入され `NetworkConfiguration` ました。
...
* **AC3/EAC3オーディオコーデックを使用したストリームの再生の有効化**
   ...
   * void alwaysUseAC3OnSupportedDevices(boolean val)in MediaPlayerクラス
...
* **ジャストインタイム — 広告マーカーに近い広告の解決**
   ...
   遅延広告解決は、各広告の時間を個別に解決するようになりました。 以前は、広告の解決は2段階のアプローチでした。再生開始前にプリロールが解決され、再生開始後に組み合わされたすべてのミッド/ポストロールスロットが含まれていました。 この機能が強化され、各広告の時間が広告キューポイントの前の特定の時間に解決されるようになりました。

   **注意：遅延広告解決がデフォルトでオフになるように変更され、明示的に有効にする必要があります。**

   この広告メタデータに関連付けられ *た遅延広告読み込みの許容値を取得するために、新しいAPIがAdvertisingMetadata::setDelayAdLoadingTolerance* に追加されました。\
   PREPARATIONの直後にシークが許可されるようになりました。広告の時間をシークすると、シークが完了する前に即座に解決されます。\
   シグナリングモードSERVER_MAPとMANIFEST_CUESがサポートされています。
...
* WebViewDebugingのデフォルト値の変更

   WebViewDebugging値は、デフォルトでFalseに設定されています。 これを有効にするには、アプリケーションでsetWebContentsDebuggingEnabled(true)を呼び出します。
...
* **SizeAvailableEventListener**
   ...
   * SizeAvailableEventのgetHeight()メソッドとgetWidth()メソッドは、それぞれ高さと幅で出力を返すようになりました。 表示縦横比は、次のように計算できます。
      ...
      Sarの幅とSarの高さに関する記憶域の縦横比は、フレームの幅とフレームの高さの計算にも使用できます。
...
この節では、TVSDK 3.10 Androidリリースで解決された問題の概要を示します。
...
* ZD#37396 — 一部のミッドロールおよびポストロールで、広告追跡イベントが見つかりません。
   * 広告追跡イベントに関する特定のケースを修正しました。
...
* ZD#36442 — 新しい再生が行われるたびに、リモートデバッグセッションが切断され、デバッグができなくなります。
   ...
   * Webビューではデフォルトでデバッグはできません。デフォルトではデバッグは有効になっていません。 MediaPlayer.getCustomAdView()から返されたオブジェクトでsetWebContentsDebuggingEnabled(true)を呼び出すことで、必要に応じてデバッグを有効にする必要があります。
...
* [TVSDK Android C++ APIドキュメント](https://help.adobe.com/en_US/primetime/api/psdk/cpp_3.5/namespaces.html) — 各Javaクラスには対応するC++クラスがあり、C++ドキュメントにはJavadocよりも詳しい説明が含まれているので、Java APIの詳細については、C++のドキュメントを参照してください。
