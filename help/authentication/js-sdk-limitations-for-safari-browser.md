...
* これらの変更は、AccessEnabler JavaScript SDK の次のバージョンに対するAdobe Primetime認証プロセスに影響を及ぼし、影響を及ぼします。v2（バージョン 2.x）、v3（バージョン 3.x）、v4（バージョン 4.x）。
...
* これらの変更は、AccessEnabler JavaScript SDK の次のバージョンのAdobe Primetime認証プロセスに影響を及ぼし、影響を及ぼします。v2（バージョン 2.x）、v3（バージョン 3.x）。
...
* 前述の問題を軽減できるアクションのリスト **3 つのバージョンすべてに適用されます** AccessEnabler JavaScript SDK の
...
>[!NOTE]
>
>AccessEnabler JavaScript SDK バージョン 2.x および AccessEnabler JavaScript SDK バージョン 3.x の場合、両方とも認証プロセスにサードパーティ cookie を使用し、Safari 11 以降の ITP およびサードパーティ cookie のため、ユーザーの認証エクスペリエンスは予期せず、ログの記録不能から予期しない期間。
...
* 自動フロー
   ...
   * JS SDK 4.0 を使用している場合、ユーザーの操作を伴わずに自動モードで試行された認証フローは、Safari 12 では成功しません。今後の JS SDK 4.1 では、自動フローに関するすべての問題が修正されることに注意してください。
...
>[!IMPORTANT]
>
>Safari 10 から Safari 12 までのセクションの上記の詳細は、Safari 13 の場合でも、まだ適用されます。
...
* Mac OS X Sierra 以前の場合：t のチェック</span>彼は&quot;**常に許可**&quot; &quot;のオプション&#x200B;**cookie と Web サイトデータ**」というエントリが追加されました。
