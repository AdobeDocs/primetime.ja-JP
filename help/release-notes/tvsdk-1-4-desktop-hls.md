...
* Zendesk #19305 — 音声ビデオの不連続性があるストリームを再生中に途切れ途切れの再生が発生しました。
...
この問題は、広告リクエストのタイムアウトに対してAuditudeSettingsに新しいAPI adRequestTimeoutを追加することで解決されました。 ユーザーは、デフォルトの10秒広告リクエストタイムアウトを上書きできるようになりました。
...
* ゼンデスク#1561 - Re: `[Adobe Primetime]` 更新：Desktop PSDKでのプログラム日時に対するHLSクライアントベースのフェイルオーバーサポート(Flash Playerバージョン16.0.0.305以降が必要)
